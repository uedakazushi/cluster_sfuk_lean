...
\text{かつ}
\ 
\lfloor n / e \rfloor + \lfloor n / f \rfloor \ge i
\}
$
\end{definition}

$\rI_{e,f}^i$は空集合かも知れないが、
${\rI'}_{e,f}^i$は決して空にはならない：

\begin{lemma}
\label{lm:setI'_non_empty}
\lean{setI'_non_empty}
\uses{df:setI'}
\leanok
${\rI'}_{e,f}^i$は空でない。
\end{lemma}
...
\text{かつ}
\ 
\lfloor n / e \rfloor + \lfloor n / f \rfloor \le i
\}
$
\end{definition}
...
\lean{setI''_finite}
\uses{df:setI'',lm:finite_of_bounded_of_Nat}
\leanok
${\rI''}_{e,f}^i$は有限集合である。
\end{lemma}
...
\label{df:b_max}
\lean{b_max}
\uses{lm:setI''_finite,lm:setI''_non_empty}
\leanok
$\bmax \coloneqq \max {\rI''}_{e,f}^i$.
\end{definition}



\begin{proposition}
\label{pr:I_is_interval}
\uses{df:b_min,df:b_max}
$\rI_{e,f}^i = [\bmin,\bmax]$
\end{proposition}

\begin{proof}
$\rI_{e,f}^i \ne \emptyset$は$\bmin$と$\bmax$を含む
\end{proof}
