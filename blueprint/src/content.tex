...
\begin{lemma}
\label{lm:setII_empty_if_not_dvd}
\lean{setII_empty_if_not_dvd}
$
i \not \equiv -1 \mod e + f
$
の時、
...
\begin{lemma}
\label{lm:setII_dichotomy}
\lean{setII_dichotomy}
\uses{df:setII}
$
\rII_{e,f}^i
$
...
\begin{align}
\varphi_{e \ell, f \ell}(n) + 1
&= \lfloor n / e \ell \rfloor + \lfloor n / f \ell \rfloor + 1 \\
&= (f k - 1) + (e k - 1) + 1 \\
&= (e + f) k - 1
\end{align}
より、
...
この時
$
\lfloor n'/e \rfloor < \lfloor n/e \rfloor
$
かつ
$
\lfloor n'/f \rfloor \le \lfloor n/f \rfloor
$
なので、
\begin{align}
\varphi_{e \ell, f \ell}(n')
 < \varphi_{e \ell, f \ell}(n)
\end{align}
を満たし、$n' \in \rII_{e \ell,f \ell}^i$に矛盾する。
同様に、
$n < n'$でも$n' \not \in \rII_{e \ell,f \ell}^i$である。
\end{proof}
